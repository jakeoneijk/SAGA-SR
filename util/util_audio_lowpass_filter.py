...
class UtilAudioLowPassFilterNVSR:
    ...
    def lowpass(
        ...
        if len(audio.shape) == 2:
            ...
            for i in range(audio.shape[0]):
                lowpassed_audio[i] = UtilAudioLowPassFilterNVSR.lowpass_filter( x=audio[i], highcutoff_freq=int(cutoff_freq), fs=sr, order=filter_order, ftype=filter_name, upsample_to_original = upsample_to_original)
        else:
            lowpassed_audio = UtilAudioLowPassFilterNVSR.lowpass_filter( x=audio, highcutoff_freq=int(cutoff_freq), fs=sr, order=filter_order, ftype=filter_name, upsample_to_original = upsample_to_original)
    ...
    def lowpass_filter(
        ...
        if len(y) != len(x):
            y = UtilAudioLowPassFilterNVSR.align_length(x, y)
        # After low pass filtering. Resample the audio signal
        y = UtilAudioLowPassFilterNVSR.subsampling(y, lowpass_ratio=highcutoff_freq / int(fs / 2), fs_ori=fs, upsample_to_original = upsample_to_original)
        return y
    ...
    def subsampling(data, lowpass_ratio, fs_ori=44100, upsample_to_original:bool = True):
        ...
        if upsample_to_original:
            ...
            if len(y) != len(data):
                y = UtilAudioLowPassFilterNVSR.align_length(data, y)
        return y
    
if __name__ == "__main__":
    util = UtilAudioLowPassFilterNVSR()
